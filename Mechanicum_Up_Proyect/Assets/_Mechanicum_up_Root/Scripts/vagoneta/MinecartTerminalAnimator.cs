...
public class MinecartTerminalAnimator : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        bool vagonetaExiste = GameObject.FindGameObjectWithTag("Vagoneta") != null;

        if (!vagonetaExiste)
        {
            // 🔥 Si no hay vagoneta, forzar oculto
            isVisible = false;
        }
        else
        {
            MinecartController cart = FindObjectOfType<MinecartController>();
            bool playerDentro = cart != null && cart.isPlayerInside;

            isVisible = playerDentro;
        }
